...
const ProductCard: React.FC<ProductCardProps> = ({ product, onHide }) => {
  ...
  return (
    <motion.div 
      ...
      className="group relative flex flex-col gap-2 sm:gap-4 rounded-xl sm:rounded-2xl bg-charcoal-surface p-2 sm:p-4 border border-charcoal/10 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 cursor-pointer"
    >
      ...
      <button
        ...
        className={`absolute top-2 right-2 sm:top-4 sm:right-4 z-10 p-1.5 sm:p-2 rounded-full backdrop-blur-sm border transition-all duration-300
          ${
            inWishlist
              ? "bg-primary text-charcoal shadow-[0_0_15px_rgba(188,132,96,0.5)] border-primary"
              : "bg-white hover:bg-white text-charcoal/40 hover:text-red-500 border-charcoal/10"
          }`}
      >
      ...
      <div className="w-full aspect-square bg-white rounded-xl sm:rounded-2xl overflow-hidden relative border border-charcoal/5 flex items-center justify-center p-4 sm:p-6">
        <img
      ...
      <div className="px-1 pb-2 mt-1 relative z-0">
        {brand && (
          <p className="text-[9px] font-bold text-charcoal/50 uppercase tracking-wide">
            {brand}
          </p>
        )}
        <h3 className={`text-[11px] sm:text-xs font-bold leading-tight mt-1 transition-colors ${outOfStock ? "text-charcoal/40" : "text-charcoal group-hover:text-primary"} line-clamp-2 min-h-[2.2rem]`}>
          {name}
        </h3>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex flex-col">
            {(product.enOferta || LAUNCH_PROMO_ACTIVE) && (
              <span className="text-[8px] text-charcoal/40 line-through font-black italic">
          ...
          <button
            className={`size-8 sm:size-9 rounded-full flex items-center justify-center transition-all duration-300 relative z-10 ${
              outOfStock 
                ...
                : "bg-charcoal/5 hover:bg-primary text-charcoal hover:text-charcoal border border-charcoal/10"
            }`}
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
